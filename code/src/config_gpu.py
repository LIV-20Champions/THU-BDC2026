"""
GPU 环境专用配置 - 恢复 V4 全部特性（VSN + MultiScale + CS）
在本地 GPU 环境下使用此配置训练，预期 score 0.023+

使用方法：
1. 备份当前 config.py: cp config.py config_cpu.py
2. 复制此配置: cp config_gpu.py config.py
3. 运行训练: python -m train --seed 42 --output_dir ./model/opt_gpu_s42
"""
...
config = {
    ...
    'num_workers': 2,  # V10 优化：数据加载并行化
    'pin_memory': True,  # GPU 优化：启用 pin_memory
    ...
    'output_dir': './model/opt_gpu_s42',
    'ensemble_model_dirs': ['./model/opt_gpu_s42', './model/opt_gpu_s123', './model/opt_gpu_s7'],
    'data_path': './data',
    'val_months': 3,
