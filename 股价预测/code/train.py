...
from training_controls import get_lr_factor, is_gradient_clipping_enabled, set_optimizer_lr
...
def train_ranking_model(model, dataloader, criterion, optimizer, device, epoch, writer):
    ...
    for batch in tqdm(dataloader, desc=f"Training Epoch {epoch+1}"):
        ...
        if batch_loss is not None:
            ...
            if is_gradient_clipping_enabled(config):
...
def main():
    ...
    best_epoch = -1
    best_scores = []  # (score, epoch, path) 用于保存 top-K 模型
    no_improve_count = 0
    ...
    try:
        for epoch in range(config['num_epochs']):
            if patience > 0 and no_improve_count >= patience:
                print(f"早停：连续 {patience} 轮未改善，在第 {epoch} 轮停止")
                break
            ...
            lr_factor = get_lr_factor(
                ...
                min_factor=config.get('lr_min_factor', 0.2),
            )
            current_lr = set_optimizer_lr(optimizer, base_lr, lr_factor)
            print(f"Learning Rate: {current_lr:.8f} (factor {lr_factor:.4f})")
            ...
            writer.add_scalar('train/lr_factor', lr_factor, global_step=epoch)

            current_selection_score = float(eval_metrics.get(selection_metric_name, 0.0))

            # 维护 top-K 模型列表（按分数降序排列，最好的在 index 0）
            rank = sum(1 for s, _, _ in best_scores if s > current_selection_score)

            if rank < num_ensemble:
                # 将 rank 位置及之后的已有模型向下移动（从末尾开始）
                for shift_idx in range(len(best_scores) - 1, rank - 1, -1):
                    old_path = best_scores[shift_idx][2]
                    new_idx = shift_idx + 1
                    if new_idx >= num_ensemble:
                        if os.path.exists(old_path) and os.path.basename(old_path) != 'best_model.pth':
                            try:
                                os.remove(old_path)
                            except OSError:
                                pass
                    else:
                        new_suffix = f'best_model_{new_idx + 1}.pth' if new_idx > 0 else 'best_model.pth'
                        new_path = os.path.join(output_dir, new_suffix)
                        if old_path != new_path:
                            ...
                            try:
                                ...
                                pass
                            best_scores[shift_idx] = (best_scores[shift_idx][0], best_scores[shift_idx][1], new_path)

                # 保存新模型
                new_suffix = f'best_model_{rank + 1}.pth' if rank > 0 else 'best_model.pth'
                new_path = os.path.join(output_dir, new_suffix)
                torch.save(model.state_dict(), new_path)
                best_scores.insert(rank, (current_selection_score, epoch + 1, new_path))

                # 删掉超出 K 的模型
                while len(best_scores) > num_ensemble:
                    removed = best_scores.pop()
                    if os.path.exists(removed[2]) and os.path.basename(removed[2]) != 'best_model.pth':
                        try:
                            os.remove(removed[2])
                        except OSError:
                            pass
            ...
            if current_selection_score > best_score:
                ...
                best_epoch = epoch + 1
                no_improve_count = 0
                print(f"新最佳模型 - {selection_metric_name}: {best_score:.6f} (top-K 模型数: {len(best_scores)})")
            else:
                no_improve_count += 1
                print(f"连续 {no_improve_count} 轮未改善 (patience={patience})")

    finally:
        writer.close()

    print(f"\n训练完成！最佳 epoch: {best_epoch}, 最佳 {selection_metric_name}: {best_score:.6f}")
    print(f"已保存 top-{len(best_scores)} 个模型用于集成预测:")
    for rank_idx, (s, ep, p) in enumerate(best_scores):
        print(f"  #{rank_idx + 1}: epoch {ep}, {selection_metric_name}={s:.6f} -> {os.path.basename(p)}")
    with open(os.path.join(output_dir, 'final_score.txt'), 'w') as f:
        ...
        f.write(f"Ensemble models: {len(best_scores)}\n")
